...
function rerenderApp() {
    imBeginLayout(); {
        ...
        imBeginLayout(COL | RELATIVE); {
            ...
            imBeginLayout(); {
                imStr(`im-dom-utils is an alternative way of writing SPAs that leverage the performance and simplicity of plan old JavaScript functions.`);
            } imEnd();
...
When you want full ownership over your entire stack
                ...
                imBeginLayout(); {
                    ...
                    imBeginRoot(newUl); {
                        imBeginRoot(newLi); imStr(`Static content`); imEnd();
                        imBeginRoot(newLi); {
                            imStr(`Multipage websites that need to be indexed, and you don't have the knowledge to set up the dev environment manually`);
                            imBeginRoot(newUl); imBeginRoot(newLi); {
                                imStr(`I had initially tried to make this page a multi-page site, and I was not having a fun time. I'll try again later.`);
                            } imEnd(); imEnd();
                        } imEnd();
                        imBeginRoot(newLi); {
                            imStr(`You need to target older browsers that don't have `);
                            imBeginRoot(newA); {
                                if (imInit()) {
                                    setAttr("href", "https://caniuse.com/requestanimationframe");
                                }
                                imStr("requestAnimationFrame");
                            } imEnd();
                        } imEnd();
                    } imEnd();
                } imEnd();
...
It mostly worked, but it had it's own problems. I could program basically anything in them, but there were a couple things that I could never implement nicely, or at all:
                `); imEnd();
                imBeginRoot(newUl); {
                    imBeginRoot(newLi); imStr(`switch statement abstraction`); imEnd();
                    imBeginRoot(newLi); imStr(`try/catch style error boundary abstraction. Or any error boundary abstraction for that matter`); imEnd();
                    imBeginRoot(newLi); imStr(`Error boundary that responds to errors in events like mouse-clicks and key presses (not just rendering errors)`); imEnd();
                    imBeginRoot(newLi); imStr(`React.Context alternative`); imEnd();
                    imBeginRoot(newLi); imStr(`Conditional rendering that actually inserted/removed DOM nodes instead of setting "display": none`); imEnd();
                    imBeginRoot(newLi); imStr(`Conditional rendering that was typesafe, and did type narrowing like TypeScript if-stamements`); imEnd();
                    imBeginRoot(newLi); imStr(`A way to define state close to where it was actually being used, to make component extraction easier`); imEnd();
                } imEnd();
                imBeginLayout(); imStr(`
